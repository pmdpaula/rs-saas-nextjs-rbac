  ...
  .refine(
    (data) => {
      if (data.domain) {
        ...
        return domainRegex.test(data.domain);
      }

      return true;
    },
    {
      message: "O domínio deve ser um endereço válido (exemplo: dominio.com).",
      path: ["domain"],
    },
  )
  .refine((data) => data.shouldAttachUsersByDomain === true && !data.domain, {
    message:
